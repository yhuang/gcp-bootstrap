data "tfe_workspace" "host_vpc_project" {
  name         = "host-vpc-project"
  organization = var.tfc_organization_name
}

data "tfe_variable_set" "host_vpc_project_credentials" {
  name         = "host-vpc-project-credentials"
  organization = var.tfc_organization_name
}
...
resource "tfe_variable" "enable_gcp_provider_auth" {
  variable_set_id = local.tfc_variable_set_id["host-vpc-project-credentials"]
